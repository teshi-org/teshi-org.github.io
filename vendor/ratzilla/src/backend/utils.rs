...
/// Returns the size in character cells that fits in the given container element.
/// Divides pixel dimensions by 10 (width) and 20 (height) to match the cell grid assumptions.
pub(crate) fn get_container_size(element: &Element) -> Size {
    let w = (element.client_width() as u16).max(20) / 10_u16;
    let h = ((element.client_height() as u16).max(20) / 20_u16).saturating_sub(1);
    Size::new(w, h)
}
